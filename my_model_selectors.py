    ...
    def select(self):
        ...
        for n_components in range(self.min_n_components,self.max_n_components+1):
            ...
            try:
                ...
                d = current_model.n_features
                N = self.X.shape[0]
                logN = np.log(N)
                
                n = n_components
                
                # As per forum posting by Katie Tiwari, number of parameters p is:
                p = n * n + 2 * n * d - 1
...
class SelectorDIC(ModelSelector):
    ...
    def select(self):
        ...
        for n_components in range(self.min_n_components, self.max_n_components+1):
            try:
                # Train the model using GaussianHMM and calculate the score logL
                current_model = GaussianHMM(n_components=n_components, covariance_type="diag", n_iter=1000,
                                            random_state=self.random_state, verbose=False).fit(self.X, self.lengths)
                logL = current_model.score(self.X, self.lengths)
                ...
                # score all other words and take their average score
                ...
                # choose current_model if current_score is the best so far.
